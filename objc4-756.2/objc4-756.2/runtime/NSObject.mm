...
static id defaultBadAllocHandler(Class cls)
{
    _objc_fatal("attempt to allocate object of class '%s' failed", 
                cls->nameForLogging());//"attempt to allocate object of class '%s' failed":尝试分配XX类的对象失败
}
...
callAlloc(Class cls, bool checkNil, bool allocWithZone=false)/*checkNil*/ /*allocWithZone*/
{
    if (slowpath(checkNil && !cls)) return nil;

#if __OBJC2__
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {//hasCustomAWZ实际意义是hasCustomAllocWithZone。这里表示有没有alloc / allocWithZone的实现（只有不是继承NSObject/NSProxy的类才为true）。
        // No alloc/allocWithZone implementation. Go straight to the allocator.
        // fixme store hasCustomAWZ in the non-meta class and 
        // add it to canAllocFast's summary
        if (fastpath(cls->canAllocFast())) {//判断当前的class是否支持快速alloc,内部调用了bits.canAllocFast默认为false
            // No ctors, raw isa, etc. Go straight to the metal. ctor:功率因素
            bool dtor = cls->hasCxxDtor(); //dtor:析构函数 dtor是用来判断当前class或者superclass是否有.cxx_destruct函数的实现。"destruct":
            id obj = (id)calloc(1, cls->bits.fastInstanceSize());//调用calloc函数，申请1块bits.fastInstanceSize()大小的内存空间
            if (slowpath(!obj)) return callBadAllocHandler(cls);//如果创建失败，也会调用callBadAllocHandler函数。
            obj->initInstanceIsa(cls, dtor);//如果创建成功，就去初始化Isa指针和dtor。
            return obj;
        }
        else {
