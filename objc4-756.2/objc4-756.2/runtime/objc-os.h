...
#define fastpath(x) (__builtin_expect(bool(x), 1))//表示x很可能不为0，希望编译器进行优化；
#define slowpath(x) (__builtin_expect(bool(x), 0))//表示x很可能为0，希望编译器进行优化——这里表示cls大概率是有值的，编译器可以不用每次都读取 return nil 指令
