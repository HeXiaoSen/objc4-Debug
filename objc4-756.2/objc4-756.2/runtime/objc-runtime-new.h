...
struct objc_class : objc_object {
    ...
    bool hasCxxCtor() {
        // addSubclass() propagates this flag from the superclass.propagates:传播
        assert(isRealized());
    ...
    size_t instanceSize(size_t extraBytes) {
        ...
        // CF requires all objects be at least 16 bytes.注意：Core Foundation 需要所有的对象的大小都必须大于或等于 16 字节。
        if (size < 16) size = 16;
        return size;
    }
