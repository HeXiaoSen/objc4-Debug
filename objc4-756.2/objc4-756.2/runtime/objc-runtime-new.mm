                              ...
                              size_t *outAllocatedSize = nil)
{
    ...
    // Read class's info bits all at once for performance
    bool hasCxxCtor = cls->hasCxxCtor();//hasCxxCtor()是判断当前class或者superclass是否有.cxx_construct 构造方法的实现
    bool hasCxxDtor = cls->hasCxxDtor();//hasCxxDtor()是判断判断当前class或者superclass是否有.cxx_destruct 析构方法的实现
    bool fast = cls->canAllocNonpointer();//canAllocNonpointer()是具体标记某个类是否支持优化的isa

    size_t size = cls->instanceSize(extraBytes);//instanceSize()获取类的大小（传入额外字节的大小），instanceSize会存储在类的 isa_t结构体中，然后经过对齐最后返回。
    if (outAllocatedSize) *outAllocatedSize = size;

    id obj;  //申请完内存空间之后，还需要再初始化Isa指针。
    if (!zone  &&  fast) {
